    ...
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="movieInput">
          ...
          <textarea name="overview" id="overview" cols="30" rows="10" value={formData.overview} onChange={handleInputChange} style={{ resize: 'none' }}></textarea>
        </div>
        ...
        <div className="movieInput">
          ...
          {formData.image_urls.map((imageUrl, index) => (
            <div key={index}>
              <input type="text" name="image_urls" value={imageUrl} onChange={(event) => handleImageUrlChange(event, index)} />
              <button type="button" onClick={() => handleRemoveImageUrl(index)}>Eliminar</button>
            </div>
          ))}
          <button type="button" onClick={handleAddImageUrl}>Agregar URL de imagen</button>
        </div>
